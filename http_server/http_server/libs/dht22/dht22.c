    ...
    float* humidity)
{
  ...
  // The 40-bit DHT22 data stream is timing-critical (microsecond-level pulse widths).
  // Under FreeRTOS we briefly enter a critical section to prevent task preemption
  // during the bit-banging read. The start signal and response check are not
  // protected to avoid blocking the scheduler for ~18 ms.
